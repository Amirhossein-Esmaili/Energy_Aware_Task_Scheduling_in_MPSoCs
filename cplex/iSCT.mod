...
/* For the current model (iSCT.mod), we do not use the following information (processor assignment and task ordering obtained from HEFT) 
as our scheduling constraints, since iSCT.mod explores solving the global integrated MILP formulation. */
int P_HEFT[Tasks] = ...;
int O_HEFT[Tasks][Tasks] = ...;
...
dexpr float idletime[v in Tasks] = Starttime[v] - sum ( k in Cores ) ( sum ( u in Tasks ) ( Mul1[k][u][v] )); // I'_v explained in the paper

// I'_k (explained in the paper) is comprised of the following two expressions:
...
main {
 ...
 /* The following commands generate a .txt file containing simulation results, including
  ...
  var ofile = new IloOplOutputFile("iSCT_"+thisOplModel.dataset_label+".txt");
  ...
  var ofile = new IloOplOutputFile("iSCT_"+thisOplModel.dataset_label+".m");
  ...
  ofile.writeln("title('iSCT Scheduling for "+thisOplModel.dataset_label+"')");
  ofile.writeln("for k = 1:NbCores");
